...
void init_complex_grid(fftwf_complex in_fftw[][N][N]) {
  ...
  for (k = 0; k < N; k++) {
  tareador_start_task("ffts1_init_complex_loop_k");
    for (j = 0; j < N; j++) {
      for (i = 0; i < N; i++)
      {
    ...
    tareador_finish_task("ffts1_init_complex_loop_k");
  }
}
...
void transpose_xy_planes(fftwf_complex  tmp_fftw[][N][N], fftwf_complex in_fftw[][N][N]) {
    ...
    for (k=0; k<N; k++) {
    tareador_start_task("ffts1_xy_planes_loop_k");
     for (j=0; j<N; j++) {
       for (i=0; i<N; i++)
       {
     ...
     tareador_end_task("ffts1_xy_planes_loop_k");
    }
}
...
void transpose_zx_planes(fftwf_complex in_fftw[][N][N], fftwf_complex tmp_fftw[][N][N]) {
    ...
    for (k=0; k<N; k++) {
    tareador_start_task("ffts1_zx_planes_loop_k");
     for (j=0; j<N; j++) {
       for (i=0; i<N; i++)
       {
     ...
     tareador_end_task("ffts1_zx_planes_loop_k");
    }
}
